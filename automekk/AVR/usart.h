...
static FILE new_std_out = FDEV_SETUP_STREAM((void *)usart_usb_transmit_printf, NULL, _FDEV_SETUP_WRITE);


// Generic USART functions
void usart_init(USART_t *usart_nr, PORT_t *usart_port, uint16_t baud, uint8_t tx_bp);
void usart_transmit_char(USART_t *usart_nr, char c);
typedef void (*usart_char_transmitter)(USART_t *usart_nr, char c);
void usart_transmit_string(USART_t *usart_nr, char *string, usart_char_transmitter transmit_char);
